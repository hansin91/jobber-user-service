import { IBuyerDocument, ISellerDocument, winstonLogger } from '@hansin91/jobber-shared';
import { config } from '@users/config';
import { Channel, ConsumeMessage } from 'amqplib';
import { Logger } from 'winston';
import { createConnection } from '@users/queues/connection';
import { createBuyer, updateBuyerPurchasedGigs } from '@users/services/buyer.service';
import { getRandomSellers, updateSellerCancelledJobs, updateSellerCompletedJobs, updateSellerOngoingJobs, updateSellerReview, updateSellerTotalGigsCount } from '@users/services/seller.service';
import { DirectMessage, publishDirectMessage } from '@users/queues/producer';
  ...
  try {
    ...
    channel.consume(jobberQueue.queue, async (msg: ConsumeMessage | null) => {
      ...
      if (type === 'create-order') {
        ...
        await updateSellerTotalGigsCount(`${gigSellerId}`, count);
    ...
    channel.consume(jobberQueue.queue, async (msg: ConsumeMessage | null) => {
      ...
      if (type === 'buyer-review') {
        ...
        const directMessagePayload: DirectMessage = {
          ...
          message: JSON.stringify({ type: 'updateGig', gigReview: msg!.content.toString() }),
